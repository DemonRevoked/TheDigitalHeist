...
- **Stage 1**: Students exploit the vulnerability to find a **key** (deployment-specific; sourced from the per-challenge key in the `keys/` folder)
...
1. **Environment Configuration**
   ...
   PORT=5101
   # KEY is deployment-specific (generated by startup.sh into keys/sc-01.key)
   # You can still override it manually with SC01_KEY if you want.
...
- Retrieve the key: (deployment-specific; pulled from `../secrets/vault.key`)
