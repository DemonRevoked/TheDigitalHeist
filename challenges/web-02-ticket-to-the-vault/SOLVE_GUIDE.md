...
### Main Payload (For Tokyo's Account)

**Important:** This payload uses `user_id: 1` because Tokyo is the first user. The flag will appear in Tokyo's `/my-captures` page.
...
**How it works:**
1. Fetches `/admin/flag` to get the flag (runs with admin's session)
2. Sends the flag to `/collector` with `user_id: 1` (Tokyo's ID)
3. Flag is stored in database linked to Tokyo's account
4. Tokyo can view it at `/my-captures`
